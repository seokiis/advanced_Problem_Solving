    ...
    for x1 in range(1, N + 1):
        for y1 in range(1, N + 1):
            if x1 == y1:
                ...
                e = (min(math.floor(c), N) - y1) // (x1 // a)
                f = (min(math.floor(d), N) - x1) // (y1 // a)
                total += e+f
            if x1 > y1:
...
if __name__ == "__main__":
    ...
    for N in range(1, 20):
        ...
        if result1 == result2:
            ...
            print(
                f"Fail with N={N}: output {result2} != expected output {result1}")
            correct = False
            break
    ...
    if not correct:
        ...
        tSpeedCompare1 = timeit.timeit(
            lambda: rightTriangle1(N), number=repeat)/repeat
        tSubmittedCode = timeit.timeit(
            lambda: rightTriangle2(N), number=repeat)/repeat
        print(f"For input N = {N}")
        print(
            f"Average running times of the submitted code {tSubmittedCode:.10f} and rightTriangle1 {tSpeedCompare1:.10f}")
        if tSubmittedCode * 200 < tSpeedCompare1:
            print("pass")
        else:
            print("fail")
        print()
        ...
        tSpeedCompare2 = timeit.timeit(
            lambda: speedCompare2(N), number=repeat)/repeat
        tSubmittedCode = timeit.timeit(
            lambda: rightTriangle2(N), number=repeat)/repeat
        print(f"For input N = {N}")
        print(
            f"Average running times of the submitted code {tSubmittedCode:.10f} and speedCompare2 {tSpeedCompare2:.10f}")
        if tSubmittedCode * 1.5 < tSpeedCompare2:
            print("pass")
        else:
            print("fail")
        print()
